...
This activity introduced students to agentic AI systems by having them extend BugHound, a multi-step debugging assistant that combines heuristic rules with an LLM. The core concept students needed to understand is that AI in an agentic system is just one component: it needs guardrails, fallbacks, and validation like any other module. Students are most likely to struggle with understanding why `_can_call_llm()` returns True for MockClient, since it has a `complete` method but cannot produce valid JSON, which causes a silent fallback that is easy to miss without reading the agent trace carefully. AI tools like Copilot were helpful for navigating unfamiliar code quickly but could be misleading when suggesting changes to the risk assessor without understanding how score, level, and should_autofix interact downstream. If a student was stuck on their guardrail change, I would ask: "What input would make this rule fire, and what input wouldn't? Can you write that as a test before you write the code?" This helps promote a sort of aligned thinking among the student and their project base. It goes to show and attest to really what students can do. 
