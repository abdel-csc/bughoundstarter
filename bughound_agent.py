...
class BugHoundAgent:
    ...
    def _can_call_llm(self) -> bool:
        if self.client is None or not hasattr(self.client, "complete"):
            return False
        # Don't treat MockClient as a real LLM — it can't return valid JSON
        if type(self.client).__name__ == "MockClient":
            return False
        return True
